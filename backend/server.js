...
app.get("/champion", async (req, res) => {
    ...
    try {
        const currentVersion = await utils.getCurrentVersionByApiIdentifier(CHAMPIONAPIIDENTIFIER);
        ...
        if(localVersion >= currentVersion) {
            ...
            .then((response) => {
                if (response.ok) {
                    ...
                    .catch((err) => {
                        throw new Error("Reason: response data could not be decrypted!");
                    });
                } else {
            ...
            .catch((err) => {
                throw new Error("Reason: could not compute the api call" + err);
            });
        }
	} catch(err) {
